package cf.wynntils.core.framework.rendering.colors;

/**
 * Created by HeyZeer0 on 19/03/2018.
 * Copyright © HeyZeer0 - 2016
 */
public class MinecraftChatColors {

    public static final CustomColor BLACK = CustomColor.fromString("000000",1);
    public static final CustomColor GRAY = CustomColor.fromString("555555",1);
    public static final CustomColor LIGHT_GRAY = CustomColor.fromString("AAAAAA",1);
    public static final CustomColor WHITE = CustomColor.fromString("FFFFFF",1);
    public static final CustomColor PURPLE = CustomColor.fromString("AA00AA",1);
    public static final CustomColor PINK = CustomColor.fromString("FF55FF",1);
    public static final CustomColor BLUE = CustomColor.fromString("5555FF",1);
    public static final CustomColor DARK_BLUE = CustomColor.fromString("0000AA",1);
    public static final CustomColor DARK_CYAN = CustomColor.fromString("00AAAA",1);
    public static final CustomColor CYAN = CustomColor.fromString("55FFFF",1);
    public static final CustomColor GREEN = CustomColor.fromString("55FF55",1);
    public static final CustomColor DARK_GREEN = CustomColor.fromString("00AA00",1);
    public static final CustomColor YELLOW = CustomColor.fromString("FFFF55",1);
    public static final CustomColor ORANGE = CustomColor.fromString("FFAA00",1);
    public static final CustomColor RED = CustomColor.fromString("3F1515",1);
    public static final CustomColor DARK_RED = CustomColor.fromString("AA0000",1);

}
