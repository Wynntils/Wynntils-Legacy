...
public class ServerEvents implements Listener {
	 ...
	 * 
	 * @see PacketIncomingFilter - Register the pipline that intercepts OUTGOING
	 *      Packets
	 * @see PacketOutgoingFilter - Check if the mod has an update available -
	...
	public void joinServer(FMLNetworkEvent.ClientConnectedToServerEvent e) {
		e.getManager().channel().pipeline().addBefore("fml:packet_handler", Reference.MOD_ID + ":packet_filter",
				new PacketIncomingFilter());
		e.getManager().channel().pipeline().addBefore("fml:packet_handler", Reference.MOD_ID + ":outgoingFilter",
				new PacketOutgoingFilter());
	 ...
	 * friend list - Check if the last user class was registered if not, make the
	 * player execute /class to register it - Updates the last class - Updates and
	...
	public void joinWorldEvent(WynnWorldEvent.Join e) {
		...
		// This codeblock will only be executed if the Wynncraft AUTOJOIN setting is
		// enabled
		// Reason: When you join a world with autojoin enabled, your current class is
		// NONE,
		// while joining without autojoin will make your current class into the selected
		// over the character selection.
		...
		if (Reference.onWars)
			return; // avoid dispatching commands while in wars/nether
	...
	/**
	 * Detects and register the current friend list of the user Called when the
	...
	public void chatMessage(ClientChatReceivedEvent e) {
		...
		if (m.find() && m.group(1).equals(McIf.player().getName())) {
			...
			FrameworkManager.getEventBus()
					.post(new WynnSocialEvent.FriendList.Remove(Sets.difference(friendsList, newFriendsList), false));
			FrameworkManager.getEventBus()
					.post(new WynnSocialEvent.FriendList.Add(Sets.difference(newFriendsList, friendsList), false));
		...
		if (!messageText.startsWith("[") && messageText.contains("guild") && messageText.contains(" ")) {
			...
			if (!splittedText[1].equalsIgnoreCase("has"))
				return;
	...
	/**
	 * Detects if the user added or removed a user from their friend list Called
	 * when the user execute /friend add or /friend remove
	 *
	...
	public void addFriend(ClientChatEvent e) {
		if (e.getMessage().startsWith("/friend add ")) {
			...
			if (PlayerInfo.get(SocialData.class).addFriend(addedFriend)) {
				FrameworkManager.getEventBus()
						.post(new WynnSocialEvent.FriendList.Add(Collections.singleton(addedFriend), true));
			}
			...
			if (PlayerInfo.get(SocialData.class).removeFriend(removedFriend)) {
				FrameworkManager.getEventBus()
						.post(new WynnSocialEvent.FriendList.Remove(Collections.singleton(removedFriend), true));
			}
	...
	public void onJoinServer(WynncraftServerEvent.Login e) {
		...
		TextComponentString msg = new TextComponentString(
				"The Wynntils servers are currently down! You can still use Wynntils, but some features may not work. Our servers should be back soon.");
	...
	/**
	 * Detects when the user enters the Wynncraft Server Used for displaying the
	 * Changelog UI
	 */
	@SubscribeEvent
	public void onJoinLobby(WynnClassChangeEvent e) {
		if (!Reference.onServer || !CoreDBConfig.INSTANCE.enableChangelogOnUpdate
				|| !CoreDBConfig.INSTANCE.showChangelogs)
			return;
		if (UpdateOverlay.isDownloading() || DownloaderManager.isRestartOnQueueFinish() || McIf.world() == null)
			return;
		...
		boolean major = !CoreDBConfig.INSTANCE.lastVersion.equals(Reference.VERSION)
				|| CoreDBConfig.INSTANCE.updateStream == UpdateStream.STABLE;
		new Thread(() -> {
	...
	private static ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor(
			new ThreadFactoryBuilder().setNameFormat("wynntils-location-updater").build());
