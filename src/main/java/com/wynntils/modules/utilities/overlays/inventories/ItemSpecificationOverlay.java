...
public class ItemSpecificationOverlay implements Listener {
    ...
    public void onItemOverlayRender(RenderEvent.DrawItemOverlay.Post e) {
        ...
        if (destinationName != null) {
            renderer.beginAbsoluteGL(e.getX(), e.getY());
            {
                ...
                color.setA(0.8f);
                renderer.drawString(destinationName, xOffset / scale, 1 / scale, color, SmartFontRenderer.TextAlignment.LEFT_RIGHT, SmartFontRenderer.TextShadow.OUTLINE);
                RenderHelper.enableGUIStandardItemLighting();
            }
            renderer.endGL();
        }
