        ...
        try{
            ...
            if (MapConfig.INSTANCE.showCompass) {
                if (MapConfig.INSTANCE.followPlayerRotation) {
                    ...
                    drawString(I18n.format("wynntils.map.overlays.minimap.north_short"), mapCentre - 2 + northDX, mapCentre - 3 + northDY, CommonColors.WHITE);
                    if (!MapConfig.INSTANCE.northOnly) {
                        drawString(I18n.format("wynntils.map.overlays.minimap.east_short"), mapCentre - 2 - northDY, mapCentre - 3 + northDX, CommonColors.WHITE);
                        drawString(I18n.format("wynntils.map.overlays.minimap.south_short"), mapCentre - 2 - northDX, mapCentre - 3 - northDY, CommonColors.WHITE);
                        drawString(I18n.format("wynntils.map.overlays.minimap.west_short"), mapCentre - 2 + northDY, mapCentre - 3 - northDX, CommonColors.WHITE);
                    }
                } else {
                    float mapCentre = (float) mapSize / 2f;
