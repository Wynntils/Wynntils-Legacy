...
public class WaypointOverviewUI extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (group == ungroupedIndex) {
            groupShift = 20;
            fontRenderer.drawString(TextFormatting.BOLD + I18n.format("wynntils.map.ui.waypoints.floating.group"), this.width/2 - 205, 43, 0xFFFFFF);
        }
        fontRenderer.drawString(TextFormatting.BOLD + I18n.format("wynntils.map.ui.waypoints.floating.icon"), this.width / 2 - 185 + groupShift, 43, 0xFFFFFF);
        fontRenderer.drawString(TextFormatting.BOLD + I18n.format("wynntils.map.ui.waypoints.floating.name"), this.width / 2 - 150 + groupShift, 43, 0xFFFFFF);
    ...
    private void setEditButtons() {
        ...
        for (int i = 0, lim = Math.min(pageHeight, getWaypoints().size() - pageHeight * page); i < lim; i++) {
            editButtons.add(new GuiButton(3 + 10 * i, this.width/2 + 85 + groupShift,54 + 25 * i,40,20, I18n.format("wynntils.map.ui.waypoints.button.edit")));
            editButtons.add(new GuiButton(5 + 10 * i, this.width/2 + 130 + groupShift, 54 + 25 * i, 40, 20, I18n.format("wynntils.map.ui.waypoints.button.delete")));
