...
public class WorldMapSettingsUI extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(textureButton = new GuiButton(99, rightAlign + 120, this.height-65, 55, 18, I18n.format(MapConfig.INSTANCE.iconTexture.displayName)));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.fontRenderer.drawString(TextFormatting.WHITE + I18n.format("wynntils.map.ui.world_map_settings.floating.textures"), (this.width - 349) / 2, this.height-60, 0xffFFFFFF);

        // Draw labels rotated 45 degrees
        GlStateManager.pushMatrix();
        GlStateManager.translate((this.width-399)/2 + 286, 29f, 0f);
        GlStateManager.rotate(-45, 0, 0, 1);
        // TODO: localise "Main map"
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (mouseButton == 0 || mouseButton == 1) {
            ...
            if (textureButton.mousePressed(mc, mouseX, mouseY)) {
                ...
                selectedTexture = IconTexture.values()[(selectedTexture.ordinal() + delta) % IconTexture.values().length];
                textureButton.displayString = I18n.format(selectedTexture.displayName);
                for (Button btn : settingButtons) {
                    btn.updateTexture(selectedTexture);
                }
                return;
            }
        }
