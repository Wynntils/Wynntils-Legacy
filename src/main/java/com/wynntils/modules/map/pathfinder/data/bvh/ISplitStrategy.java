...
import com.wynntils.modules.map.pathfinder.data.bvh.NSplitStrategy.NSplitFactory;
...
public interface ISplitStrategy {
	 ...
	 */
	<T extends IBoundingBox> List<List<T>> split (List<T> elements);
	...
	public enum StrategyFactory {
		...
		public static StrategyFactory fromString (final String name) {
			return Arrays.stream(values()).filter(v -> v.toString().equals(name)).findAny().orElse(null);
		}
	}
}
