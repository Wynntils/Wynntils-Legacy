...
public class CurrentMusicDisplayer implements Listener {
    ...
    public void onDrawInGameMenu(GuiOverlapEvent.IngameMenuOverlap.DrawScreen e) {
        ...
        float size = r.drawString((MusicManager.getPlayer().getCurrentMusic() != null ? MusicManager.getPlayer().getCurrentMusic().getName().replace(".mp3", "") : I18n.format("wynntils.music.overlays.now_playing.nothing_playing")), 0, 155, CommonColors.WHITE, SmartFontRenderer.TextAlignment.MIDDLE, SmartFontRenderer.TextShadow.NORMAL);
        ...
        r.drawString((MusicManager.getPlayer().getCurrentMusic() != null ? MusicManager.getPlayer().getCurrentMusic().getName().replace(".mp3", "") : I18n.format("wynntils.music.overlays.now_playing.nothing_playing")), 0, 155, CommonColors.WHITE, SmartFontRenderer.TextAlignment.MIDDLE, SmartFontRenderer.TextShadow.NORMAL);
        ...
        if(MusicManager.getPlayer() != null) {
            if(MusicManager.getPlayer().isPaused()) {
                ...
                if(x >= -7 && y >= -186 && x <= 7 && y <= -171) e.getGui().drawHoveringText(I18n.format("wynntils.music.overlays.now_playing.play"), e.getMouseX(), e.getMouseY());
            }else{
                r.drawRect(Textures.UIs.hud_overlays, -8, 170, 34, 0, 16, 16);
                if(x >= -7 && y >= -186 && x <= 7 && y <= -171) e.getGui().drawHoveringText(I18n.format("wynntils.music.overlays.now_playing.pause"), e.getMouseX(), e.getMouseY());
            }
        }
