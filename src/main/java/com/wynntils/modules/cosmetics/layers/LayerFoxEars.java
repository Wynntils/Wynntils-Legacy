...
public class LayerFoxEars implements LayerRenderer<AbstractClientPlayer> {
    ...
    private final ModelRenderer bipedFoxEarL;
    private final ModelRenderer bipedFoxEarR;

    public LayerFoxEars(RenderPlayer playerRendererIn) {
        this.playerRenderer = playerRendererIn;
        this.bipedFoxEarL = new ModelRenderer(playerRendererIn.getMainModel(), 50, 0);
        this.bipedFoxEarR = new ModelRenderer(playerRendererIn.getMainModel(), 50, 0);
    }
    ...
    public void renderModelL(AbstractClientPlayer player, ModelBase model, float scale) {
        ...
        if (player.isSneaking()) {
            bipedFoxEarL.addBox(0.0F + 4F, -5.0F + 5F / 2F, 1f, 6, 6, 1);

        } else {
            bipedFoxEarL.addBox(0.0F, -5.0F, -1.0F, 6, 6, 1);
        }
    ...
    public void renderModelR(AbstractClientPlayer player, ModelBase model, float scale) {
        ...
        if (player.isSneaking()) {
            bipedFoxEarR.addBox(0.0F + 4F, -5.0F + 5F / 2F, 1f, 6, 6, 1);

        } else {
            bipedFoxEarR.addBox(0.0F, -5.0F, -1.0F, 6, 6, 1);
        }
