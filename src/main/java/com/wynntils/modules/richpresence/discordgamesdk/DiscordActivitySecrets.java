...
public class DiscordActivitySecrets extends Structure {
    ...
    public DiscordActivitySecrets(byte match[], byte join[], byte spectate[]) {
        super();
        if ((match.length != this.match.length)) throw new IllegalArgumentException("Wrong array size !");
        this.match = match;
        if ((join.length != this.join.length)) throw new IllegalArgumentException("Wrong array size !");
        this.join = join;
        if ((spectate.length != this.spectate.length)) throw new IllegalArgumentException("Wrong array size !");
