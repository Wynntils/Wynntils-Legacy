...
public class SmartFontRenderer extends FontRenderer {
    ...
    public float drawString(String text, float x, float y, CustomColor customColor, TextAlignment alignment, TextShadow shadow) {
        ...
        switch (alignment) {
            ...
            default:
                ...
                posX = x;
                posY = y;
                return drawChars(text, colour, false, shadow);
        }
    ...
    private float drawChars(String text, int color, boolean forceColor, TextShadow shadow) {
        if (text.isEmpty()) return -CHAR_SPACING;
        float textLength = 0f;
        boolean obfuscated = false;
        for (int index = 0; index < text.length(); index++) {
            if (text.charAt(index) == '§' && index + 1 < text.length()) {
                Integer colorToRender = color;
                colorToRender = decodeCommonColor(text.charAt(index + 1));
