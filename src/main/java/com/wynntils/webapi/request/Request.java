...
public class Request {
    ...
    /**
     * Callback called with the raw bytes from request and the connection itself.
     * Return value is interpreted as in {{@link #handle(Predicate) handle}}.
     * The connection will be `null` if loading from cache
     */
    ...
    public Request handleString(ThrowingBiPredicate<URLConnection, String, IOException> handler, Charset charset) {
    ...
    public Request handleJson(ThrowingBiPredicate<URLConnection, JsonElement, IOException> handler) {
    ...
    public Request handleJsonObject(ThrowingBiPredicate<URLConnection, JsonObject, IOException> handler) {
        return handleJson((conn, j) -> {
            if (!j.isJsonObject()) return false;
    ...
    public Request handleJsonArray(ThrowingBiPredicate<URLConnection, JsonArray, IOException> handler) {
        return handleJson((conn, j) -> {
            if (!j.isJsonArray()) return false;
